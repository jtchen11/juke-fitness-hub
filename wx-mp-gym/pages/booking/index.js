...
Page({
  ...
  loadUserBenefits() {
    ...
    if (u.userId) { request.get('/api/members/' + u.userId + '/benefits', null, (r) => { if (r.freePtRemaining) this.setData({ freeRemaining: r.freePtRemaining }); }); }
  },
  ...
  confirmBooking() {
    ...
    if (t.type === 'group') {
      ...
      request.post('/api/class-bookings', { classId: t.course.id, memberId: app.globalData.userInfo.userId }, (r) => {
      ...
      request.post('/api/personal-trainings', {
        memberId: app.globalData.userInfo.userId, trainerId: t.coach.id,
  ...
  onLoginClose() { this.setData({ showLoginModal: false }); wx.navigateBack(); },
