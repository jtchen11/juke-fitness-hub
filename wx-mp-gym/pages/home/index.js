...
Page({
  ...
  loadRecommendedCourses() {
    ...
    request.get('/api/classes', { sort: 'enrolled', limit: 6 }, function(res) {
      ...
      list.forEach(function(c) {
        c.cover = _this.resolveImageUrl(c.coverImage, 'group_class');
      });
  ...
  goBooking(e) { if (!app.isLoggedIn()) { wx.navigateTo({ url: '/pages/login/login' }); return; } wx.navigateTo({ url: '/pages/booking/index?id=' + e.currentTarget.dataset.id }); },
